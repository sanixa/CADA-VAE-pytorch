...
class DATA_LOADER(object):
    def __init__(self, dataset, aux_datasource, device='cuda'):
        ...
        data_path = str(project_directory) + '/model/data'
        print('Data Path')
        print(data_path)
        ...
        if self.dataset == 'CUB':
            ...
            self.datadir = self.data_path + '/plant/'
    ...
    def read_matdataset(self):
        ...
        if self.dataset == 'AWA2':
            temp_dataset = 'AwA2'
        temp_path= '/home/uscc/cvamc/data/' + temp_dataset + '/learned_class_attr.npy' 
        temp_attr = np.load(temp_path, allow_pickle=True)

        scaler = preprocessing.MinMaxScaler()
        temp_attr = scaler.fit_transform(temp_attr)
        
        ###############------------------------------------------

        if self.auxiliary_data_source == 'attributes':
            self.aux_data = torch.from_numpy(matcontent['att'].T).float().to(self.device)
            #self.aux_data = torch.from_numpy(temp_attr).float().to(self.device)
        else:
            if self.dataset != 'CUB':
